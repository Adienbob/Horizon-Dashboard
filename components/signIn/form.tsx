...
export default function Form() {
   ...
   return (
      <section id="signIn" className="bg-(--component-bg) dark:bg-(--main-bg)  text-(--primary) text-sm relative md:px-20 col-span-1 lg:mb-35">
         <button onClick={() => setTheme(prev => prev === "dark" ? "light" : "dark")} className="z-250 p-5 rounded-full fixed bottom-[30px] right-[30px] border border-[#6A53FF]" style={{ background: "linear-gradient(90deg, #868CFF 0%, #4318FF 100%)" }}>
         ...
         <div className="forContainer">
            <div className="mb-6 mt-10">
               ...
               <button className="flex gap-2.5 w-full bg-(--main-bg) dark:bg-(--lessons-btns-bg) rounded-2xl justify-center">
                  <Image 
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="mt-[26px] font-medium [&_span]:text-(--primary-blue) [&_label:not(:last-child)]:grid grid gap-6 [&_input]:rounded-2xl [&_input]:pt-[17px] [&_input]:pb-[19px] [&_input]:border [&_input]:border-(--toggle-off) [&_input]:pl-6">
               ...
               <div className="font-normal leading-5 flex justify-between my-2.5">
                  ...
                  <Link href={"/"} className="text-(--btn-color) font-medium">
                     Forgot password?
                  </Link>
               </div>
               <button type="submit" className="bg-(--primary-blue) text-white py-[15px] w-full rounded-2xl font-bold">Sign In</button>
               <p className="font-normal leading-[26px] text-(--primary) dark:text-(--secondary)">Not registered yet? <Link href={"/"} className="text-(--btn-color) font-bold">Create an Account</Link></p>
            </form>
         </div>
      </section>
   )
